...
//  freshApp
//
//  Created by Forrest Buhler on 10/10/21.
//
...
class TextFieldAlertViewController: UIViewController {
  ...
  ///   - isPresented: binding to be set to false when the alert is dismissed (`Done` button tapped)
    init(title: String, message: String?, text: Binding<String?>, isPresented: Binding<Bool>?, email: String, errorString: Binding<String>) {
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    presentAlertController(email: email)
  }

    func presentAlertController(email: String) {
...
struct TextFieldAlert {
  ...
  func dismissable(_ isPresented: Binding<Bool>) -> TextFieldAlert {
    TextFieldAlert(title: title, message: message, text: $text, isPresented: isPresented, email: email, errorString: _errorString)
  }
}
...
extension TextFieldAlert: UIViewControllerRepresentable {
  ...
  func makeUIViewController(context: UIViewControllerRepresentableContext<TextFieldAlert>) -> UIViewControllerType {
    TextFieldAlertViewController(title: title, message: message, text: $text, isPresented: isPresented, email: email, errorString: $errorString)
  }
